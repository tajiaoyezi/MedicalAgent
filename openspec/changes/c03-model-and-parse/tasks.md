...
- [ ] 1.5 新增迁移：创建 `document_parse_jobs` 表，含 `document_id`、`document_version`、`tenant_id`（多租户隔离）、状态字段（pending/parsing/succeeded/failed 及内部子状态）、`failure_reason`、时间戳、操作者；验证：可创建并按 (document_id, version) 查询作业
