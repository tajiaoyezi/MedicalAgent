...
- [x] 15.1 真实接入测试：面板经 c02 真实 ONLYOFFICE 宿主桥 SDK 完成挂载→读取选区→确认→`replaceSelection` 写回→保存回调落 `document_versions` 全链路；验证：在真实 ONLYOFFICE 部署上选区润色可写回并生成新版本（对应 §24.2 写回验收）。【已端到端验证：起 `onlyoffice/documentserver:8.2` 容器，DS 依赖 e2e `12-onlyoffice-live`（`DS_LIVE=1` 开启、默认跳过、x3 稳定）实证 **真实 DS 渲染合法 docx → c02 桥挂载（plugin→host 经 `window.top`、host 取 `event.source`=插件 window 回发命令）→ `getDocumentType` 往返 → `replaceSelection`(PasteText) 写回 → `saveDocument` arm + 后端触发 DS forcesave(status=6) → 保存回调落 `source=ai_writeback` 新版本**。期间修复三处 c02 真实 DS 阻塞（修法见下，均属 c02 桥而非 c05 业务）：① 插件未正确打包——`config.json` 入口须为 HTML，新增 `index.html` 载 ONLYOFFICE 插件 SDK `plugins.js`（离线打包）+ `code.js`，否则插件根本不执行；② 桥消息路由——插件嵌套于 DS 插件 iframe，`window.parent.postMessage` 不达宿主，改 `window.top.postMessage`，宿主以就绪包 `event.source` 锁定插件 window 回发命令；③ 写回保存——`Api.Save` 仅产生 status=2 user_edit，新增 `editor.Service.Forcesave`（命令服务 `forcesave`）在 arm 后触发 status=6 forcesave 落 ai_writeback（插件不再 `Api.Save` 以免与 forcesave 去重）。回归：`smoke:onlyoffice`/`smoke:editor-authz`(15) 绿、默认 e2e 104+1skip 绿。】
