...
- [x] 4.3 实现批量上传逐项落库（每份文档独立入库记录、解析状态、索引状态），验证：一次上传 N 份生成 N 条入库记录（对应「批量上传逐项落库」Scenario）。
