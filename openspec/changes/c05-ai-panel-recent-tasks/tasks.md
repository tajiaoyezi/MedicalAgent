...
- [ ] 15.1 真实接入测试：面板经 c02 真实 ONLYOFFICE 宿主桥 SDK 完成挂载→读取选区→确认→`replaceSelection` 写回→保存回调落 `document_versions` 全链路；验证：在真实 ONLYOFFICE 部署上选区润色可写回并生成新版本（对应 §24.2 写回验收）。【部分验证：已起 `onlyoffice/documentserver:8.2` 容器，新增 DS 依赖 e2e `12-onlyoffice-live`（`DS_LIVE=1` 开启、默认跳过）实证**真实 DS 渲染合法 docx + c02 编辑器/下载/回调链路（DS 真实回调 `/api/editor/callback`）+ DS 加载 c02 桥插件 + c05「文档打开默认展示面板」在真实 DS 上触发**。**阻塞项（c02 后续，非 c05）**：c02 桥插件 `window.parent.postMessage` 无法穿透 ONLYOFFICE 嵌套插件 iframe 到达宿主（host↔plugin 往返失效，`medoffice-bridge-ready` 不达、`getSelectedText`/`replaceSelection` 无响应），故「选区→程序化 `replaceSelection`→保存回调落版本」端到端无法经桥在真实 DS 跑通——需先修 c02 桥↔插件消息路由（改用 `window.top` 等正确传输）。c02 保存回调→`document_versions` 机制本身由 `smoke:onlyoffice`/`smoke:editor-authz` 模拟回调覆盖，且本测试实证真实 DS 会真实回调。故本项仍留空待 c02 修复后补完写回端到端。】
