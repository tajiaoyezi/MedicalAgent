# Visual Parsing Service Specification

## Purpose

文档视觉解析服务抽象：用于扫描 PDF、图片、复杂 PDF、图表、表格与版式结构抽取，底层实现（传统 OCR / 多模态大模型 / 版面分析 / 表格识别 / 第三方 API / 私有化服务）对上层透明，上层仅依赖统一结构化输出契约（文本、页码、段落、坐标、标题层级、表格结构、图片位置、页眉页脚、置信度、失败原因、chunk 定位）并持久化于 `document_visual_parse_results`，直接作为 `document-parsing` 的 chunk 切分输入；provider 配置存 `visual_parse_providers`，公网 / 私有化双入口独立配置；公网解析前强制消费 c09 `redaction-gateway` 脱敏门禁（本期未接入时公网默认拒绝、改走私有化降级）；表格/页码质量指标（页码定位 ≥ 90%、表格结构识别 ≥ 85%、引用源页码误差 ≤ 1 页）的最终达标判定随 c09 Evals 跑批完成；结果继承来源文档级权限（由 `document_permissions` 派生）、不引入独立 `chunk_acl` 列、不作 §11.9 RAG 检索过滤维，并写 `audit_logs` 可审计。
