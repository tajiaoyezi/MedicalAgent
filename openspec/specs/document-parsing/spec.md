...
文档上传或经 ONLYOFFICE 保存回调创建新版本后的异步解析流水线：在 `document_parse_jobs` 维护 `pending/parsing/succeeded/failed` 状态机（失败记 `failure_reason`、可重试、不写半成品 chunk）；文本型文档切分写 `document_chunks`、经 `Embed` 写 `embeddings` 并发出「索引就绪」事件（c03 仅产出 chunk+embedding 不构建检索索引，BM25/向量索引由 c04 消费该事件构建）；扫描/复杂文档以 `visual-parsing-service` 的结构化输出为切分输入；chunk 元数据完整性（§16.3）与 `chunk_acl`/`tenant_id` 继承（chunk 级可严于文档级、embeddings 经 `chunk_id` 外键回连且无独立 `tenant_id` 列）；作业生命周期仅落 `audit_logs`，不写 `document_events`；c03 作为 `document_events` 全 6 类触发源的唯一重解析/索引消费方（纯消费、不产生）。
