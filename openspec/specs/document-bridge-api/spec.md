# Document Bridge API Specification

## Purpose

医疗 AI 面板与 ONLYOFFICE 编辑器之间的 Bridge API：读取类、写回类、面板控制类方法，以及 token / 权限 / 租户 / 审计安全要求。写回方法在此定义，不含 AI 写回确认弹窗 UI 与具体 AI 技能（归 c05）。
