# AI Writeback Confirmation Specification

## Purpose
AI 写回确认网关：写回前确认面板四要素（原文 / 修改后 / 修改说明 / 影响范围）与「应用到文档 / 生成副本 / 取消」操作、§9.6 默认写回策略矩阵（选区替换 / 全文生成副本 / 校对批注 / 翻译副本 / 排版新版本）、写回时文档冲突与权限校验、高风险写回的人工确认链路（doctor / reviewer 裁决）、(`subject_type`, `subject_id`) 多态确认键覆盖 document / message / translation_job 三类产生方、写回确认记录 `writeback_confirmations` 与审计留痕。本能力为 `risk_type` 高风险判定与 `writeback_confirmations` 记录的唯一 owner，c09 仅引用消费做统一验收与审计。
