# AIMed Assistant Specification

## Purpose
AIMed 学术助手的六大模式与数据源约束、模式切换与发送按钮状态机、智能模式匹配与优先级/复合任务与无关问题处理、文件上传与管理（§8.6）、答案生成过程与结构（可溯源、默认草稿）、AIMed 高风险答案下发前经 c05 高风险确认链路（subject=message）人工确认、公网模型调用前 PHI/PII 脱敏门禁、答案操作栏、feedbacks 多来源反馈表（本 change 为建表 owner）、AIMed 会话上下文与文档发起、会话基座 module/source 维供 kb_qa 复用。
