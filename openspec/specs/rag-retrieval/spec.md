...
## Purpose
RAG 检索编排：查询改写→按模式选数据源→召回→六维权限与多租户过滤→BM25 / 向量混合与 rerank·RRF→压缩与引用关联；消费 c03『索引就绪』事件构建 BM25+向量检索索引（c04 为索引侧唯一消费方）；chunk 元数据与 chunk_acl 过滤（消费 c03 物理列，不建列）；公网模型调用前脱敏门禁。
