...
## Purpose
共享 UI 设计系统：design token 与蓝白/绿白/科技深色三套主题、基础组件库、门户外壳布局、医疗信任 UI 原语（引用角标/风险条/免责声明/合规徽标/写回确认卡呈现骨架）与通用态，供 c01 已实现页面与 c02–c08 后续前端 phase 统一复用（仅呈现层，数据/判定归各 owner）。
