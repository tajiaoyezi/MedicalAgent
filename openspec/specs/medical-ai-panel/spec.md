# Medical AI Panel Specification

## Purpose
ONLYOFFICE 编辑器右侧医疗 AI 面板：面板挂载与三类入口（右侧固定图标 / 顶部医疗空间按钮 / 选区浮层）、文档打开后默认展示（本能力为该触发唯一 owner）、Word 文档 P0 AI 功能集与 PDF/OFD P0 子集、从当前文档发起 AIMed 与发起医学翻译的入口与上下文传递、面板公网模型调用前经 c09 redaction-gateway 的 PHI/PII 脱敏门禁。面板经 c02 Bridge 控制（openAIPanel / closeAIPanel / runAIPanelSkill / streamContentToEditor），写回经 `ai-writeback-confirmation` 确认，不直接读写编辑器底层。
