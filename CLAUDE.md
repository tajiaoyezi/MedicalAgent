...
前后端真实联调用例（`@playwright/test`，配置 `playwright.config.ts`，用例在 `e2e/`）。**前置**：docker 依赖已起 + `npm run dev:api`（:3001）+ `npm run dev:web`（:5173）三者均在跑；单租户 seed（admin/admin123、user/user123）。**改后端代码后务必重启 `dev:api` 再测**（编辑器/AIMed 等路由按当前二进制注册，旧进程会 404）。
