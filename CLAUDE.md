...
前后端真实联调用例（`@playwright/test`，配置 `playwright.config.ts`，用例在 `e2e/`）。**前置**：① 首次 `npm i` 后 `npx playwright install chromium` 装浏览器；② docker 依赖已起 + `npm run dev:api`（:3001）+ `npm run dev:web`（:5173）三者均在跑；③ 单租户 seed（admin/admin123、user/user123）。**改后端代码后务必重启 `dev:api` 再测**（编辑器/AIMed 等路由按当前二进制注册，旧进程会 404）。

```bash
npm run test:e2e                   # 跑全部 E2E（chromium，串行 workers=1；setup 先登录落 storageState）
npm run test:e2e:report            # 打开上次 HTML 报告
npm run typecheck:e2e              # 仅类型检查 e2e（tsc --noEmit -p tsconfig.json，根 tsconfig 只覆盖 e2e/ + playwright.config.ts）
npx playwright test 04-documents   # 只跑某文件
```

> IDE（GoLand/VS Code）红波浪线：根 `tsconfig.json` + `@types/node` 提供 e2e 的类型解析；用例运行不依赖它（Playwright 自带 esbuild 转译），apps/web 有独立 tsconfig 互不影响。

覆盖：认证/RBAC、门户外壳与导航、4 占位页、文档中心（tab/上传/删除/打开）、最近任务、管理后台四 tab、编辑器/预览宿主，以及 c04 AIMed/RAG/Citation 后端 API 契约（无前端，用 `request` 夹具经 :5173 反代打 :3001）。运行期产物（截图/storageState/报告）落 `e2e/.artifacts|.auth|.report|.test-results`，均已 gitignore。**陷阱**：所有 storageState 用例共享同一服务端会话，登出/会话销毁类用例必须用独立会话隔离（见 `e2e/01-auth.spec.ts` 的退出登录组），否则级联污染后续用例。
